...
subplot(3,4,4)
plot(hd_vector,ego_curve,'k','linewidth',3)
xlabel('angle (rad)')
axis([0 2*pi -inf inf])
box off
title('EgoBearing')
...
ego_param = param_full_model(numel(param_full_model)-n_ego_bins+1:numel(param_full_model));
...
scale_factor_pos = mean(exp(speed_param))*mean(exp(hd_param))*mean(exp(ego_param))*50;
scale_factor_hd = mean(exp(speed_param))*mean(exp(pos_param))*mean(exp(ego_param))*50;
scale_factor_spd = mean(exp(pos_param))*mean(exp(hd_param))*mean(exp(ego_param))*50;
scale_factor_ego = mean(exp(speed_param))*mean(exp(hd_param))*mean(exp(pos_param))*50;
...
set(gca,'XTickLabel',{'PHSE','PHS','PHE','PSE','HSE','PH','PS','PE','HS',...
    'HE','SE','P','H','S','E'});
